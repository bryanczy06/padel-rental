...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: t('dashboard.rentedNow'), value: stats.rented,    icon: CircleDot,    color: 'text-brand-600 dark:text-brand-400',  bg: 'bg-brand-50 dark:bg-brand-900/30'  },
    { label: t('dashboard.available'), value: stats.available, icon: CircleDot,    color: 'text-gray-600 dark:text-gray-400',   bg: 'bg-gray-100 dark:bg-gray-800'  },
    { label: t('dashboard.inRepair'),  value: stats.repair,    icon: AlertTriangle, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-900/30'  },
    { label: t('dashboard.todayRentals'), value: stats.today,  icon: TrendingUp,   color: 'text-blue-600 dark:text-blue-400',   bg: 'bg-blue-50 dark:bg-blue-900/30'   },
  ]
    ...
    <Layout>
      <div className="flex flex-col gap-6">
        <div>
          ...
          {activeClub && <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">{activeClub.name}</p>}
        </div>
        ...
        {/* Overdue alert */}
        {overdue.length > 0 && (
          <div className="card border border-amber-200 bg-amber-50 dark:bg-amber-900/30">
            <div className="flex items-center gap-2 mb-3">
              <AlertTriangle size={18} className="text-amber-600 dark:text-amber-400" />
              <p className="font-semibold text-amber-700 dark:text-amber-300">{t('dashboard.overdueAlert')} ({overdue.length})</p>
            </div>
            <div className="flex flex-col gap-2">
              {overdue.map(r => {
                const hrs = ((Date.now() - new Date(r.started_at)) / 3600000).toFixed(1)
                return (
                  <div key={r.id} className="flex items-center gap-3 p-2 bg-white dark:bg-gray-900 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 dark:text-gray-500">{r.rackets?.name}</p>
                    </div>
        ...
        {/* Period selector */}
        <div className="flex bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-1 gap-1 self-start">
          {PERIODS.map(p => (
            <button key={p.key} onClick={() => setPeriod(p.key)}
              className={`px-4 py-1.5 text-sm font-medium rounded-lg transition-colors ${
                period === p.key ? 'bg-brand-600 text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300'
              }`}>
              {p.label}
            </button>
          ))}
        ...
        {/* Chart */}
        <div className="card">
          <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-4">השכרות — {PERIODS.find(p => p.key === period)?.label} אחרונים</h2>
        ...
        {revenueChart.length > 0 && (
          <div className="card">
            <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-4">הכנסות — {PERIODS.find(p => p.key === period)?.label} אחרונים</h2>
            <ResponsiveContainer width="100%" height={180}>
        ...
        <div className="card">
          <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-1">לקוחות חדשים מול חוזרים — {PERIODS.find(p => p.key === period)?.label} אחרונים</h2>
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">חוזר = לקוח שביצע צ׳ק אין גם לפני תחילת התקופה</p>
          {customerSplit.every(s => s.value === 0) ? (
            <p className="text-sm text-gray-400 dark:text-gray-500 py-6 text-center">אין נתוני צ׳ק אין בתקופה זו</p>
          ) : (
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <div className="flex flex-col gap-3 flex-1 w-full">
                <div className="flex items-center gap-3 p-3 bg-green-50 rounded-xl">
                  ...
                  <div>
                    <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{customerSplit[0]?.value ?? 0}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">לקוחות חדשים</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-xl">
                  ...
                  <div>
                    <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{customerSplit[1]?.value ?? 0}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">לקוחות חוזרים</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-900 dark:text-gray-100">זמן השכרה ממוצע — {PERIODS.find(p => p.key === period)?.label} אחרונים</h2>
            {avgDurationMin != null && (
              <span className="text-sm font-bold text-brand-700 dark:text-brand-300 bg-brand-50 dark:bg-brand-900/30 px-3 py-1 rounded-lg">
                ממוצע: {avgDurationMin} דק׳
              </span>
            )}
          </div>
          {avgDurationMin == null ? (
            <p className="text-sm text-gray-400 dark:text-gray-500 py-6 text-center">אין השכרות שהוחזרו בתקופה זו</p>
          ) : (
            <ResponsiveContainer width="100%" height={180}>
