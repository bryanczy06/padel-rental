    ...
    <Layout>
      <div className="max-w-md mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">{t('return.title')}</h1>
        </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="card border border-brand-100">
              <div className="flex items-center gap-3 mb-3">
                <CircleDot size={18} className="text-brand-600 dark:text-brand-400" />
                <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="ms-auto text-xs text-gray-400 dark:text-gray-500">{duration()}</span>
              </div>
            </div>

            {/* Condition */}
            <div className="card flex flex-col gap-3">
              <p className="font-semibold text-gray-900 dark:text-gray-100">{t('return.condition')}</p>
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => setCondition('good')}
                  className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 text-sm font-medium
                    ${condition === 'good' ? 'border-brand-500 bg-brand-50 dark:bg-brand-900/30 text-brand-700 dark:text-brand-300' : 'border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-gray-300 dark:border-gray-600'}`}>
                  <CheckCircle2 size={24} className={condition === 'good' ? 'text-brand-600 dark:text-brand-400' : 'text-gray-400 dark:text-gray-500'} />
                  {t('return.good')}
                </button>
                <button
                  onClick={() => setCondition('damaged')}
                  className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 text-sm font-medium
                    ${condition === 'damaged' ? 'border-red-400 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-gray-300 dark:border-gray-600'}`}>
                  <AlertTriangle size={24} className={condition === 'damaged' ? 'text-red-500 dark:text-red-400' : 'text-gray-400 dark:text-gray-500'} />
                  {t('return.damaged')}
                </button>
              </div>
        ...
        {step === STEP.DONE && (
          <div className="card flex flex-col items-center gap-4 py-10 text-center">
            <div className={`h-16 w-16 rounded-full flex items-center justify-center
              ${condition === 'good' ? 'bg-brand-50 dark:bg-brand-900/30' : 'bg-amber-50 dark:bg-amber-900/30'}`}>
              {condition === 'good'
                ? <CheckCircle2 size={36} className="text-brand-600 dark:text-brand-400" />
                : <AlertTriangle size={36} className="text-amber-500 dark:text-amber-400" />}
            </div>
            <div>
              <p className="font-bold text-gray-900 dark:text-gray-100 text-xl">{t('return.success')}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                {racket.name} · {rental.customers?.full_name}
              </p>
              {condition === 'damaged' && (
                <p className="text-sm text-amber-600 dark:text-amber-400 mt-1 font-medium">→ נשלח לתיקון / Sent to repair</p>
              )}
            </div>
